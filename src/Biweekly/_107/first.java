...
//        out.println(maximumNumberOfStringPairs(new String[]{"aa","ab"}));
        out.println(longestString(1, 1, 1));
        out.println(longestString(2, 5, 1));
        out.println(longestString(50, 50, 2));

    }


    public static int longestString(int x, int y, int z) {
        return go(x, y, z, '0',new HashMap<>());
    }
    static int go(int x, int y, int z,char last, HashMap<String, Integer> memo){
        ...
        if(last == '0'){
            if(x > 0) max = Math.max(go(x-1, y, z, 'x',memo) + 2, max);
            if(y > 0) max = Math.max(go(x, y - 1, z, 'y',memo) +2 , max);
            if(z > 0) max = Math.max(go(x, y, z - 1 , 'z',memo) + 2, max);
        }else{
            if(last == 'x'){
                if(y > 0)      max = Math.max(go(x, y-1, z, 'y', memo) + 2  , max);
            }
