...
public class all {
    public static void main(String[] args) {
        int[] arr = new int[]{1 ,2};
        SegmentTree seg = new SegmentTree(0,arr.length);
        for(int i = 0; i < arr.length; i++){
            seg.update(i, arr[i]);
        }
        out.println(Arrays.toString(seg.val));
        out.println(seg.query(1,1));
    }
    ...
    private static class SegmentTree
    {
        ...
        public void update(int index, int delta)
        {
            ...
            for (node >>= 1; node > 0; node >>= 1)
                val[node] = comb((int) val[node << 1], (int) val[(node << 1) + 1]);
        }
...
//        Collections.binarySearch(, 2 );
   ...
   static int go(int[] nums, int i, int j, TreeMap<String,Integer> memo){
        ...
        if(nums[i] == j + 1){
            ...
            for(int a = 0; a < 3; a++){
                ...
                min = Math.min(min, go(nums, i + 1, a, memo) + 1 );
