...
class _130 {
    ...
    public static void main(String[] args) {
        String s = "fabccddg";
        out.println(minimumSubstringsInPartition("abababaccddb"));
        String ss = "acccbbb"; //2
        out.println(minimumSubstringsInPartition(ss));
    }
    ...
    public static int minimumSubstringsInPartition(String s) {
        ...
        Arrays.fill(dp, -1);
        return go(s.toCharArray(), 0, s, dp);
    }

    private static int go(char[] arr, int ind, String s, int[] dp) {
        if (ind == arr.length) return 0;
        if (dp[ind] != -1) return dp[ind];
        int min = Integer.MAX_VALUE;
        for (int i = ind + 1;i < arr.length; i++) {
            if (isGoodPartition(s.substring(ind, i + 1))) {
                min = Math.min(1 + go (arr, i + 1, s,dp ), min);
            }
        }
        return dp[ind] = min == Integer.MAX_VALUE ? 0 : min;
    }
    public static boolean isGoodPartition(String s) {
    ...
    public static int maxPointsInsideSquare(int[][] points, String s) {
        ...
        out.println(isGood(0, points, s));
        out.println(isGood(1, points, s));

        out.println(isGood(2, points, s));

        out.println(isGood(3, points, s));

        out.println(isGood(4, points, s));


        while (start <= end) {
            if (maxPoints == points.length) break;
            int r = start + (end - start) / 2;
            if (isGood(r, points, s)) {
                start = r + 1;
                answer = r;
            } else {
                end = r - 1;
    ...
    private static boolean isGood(int r, int[][] points, String s) {
        HashSet<Character> set = new HashSet<>();
        for (int i = 0; i < points.length; i++) {
            int[] point = points[i];
            if (isValidSquare(point[0], point[1], r)) {
        ...
        int size = set.size();
        maxPoints = Math.max(maxPoints, size);
        return true;
    }
    public static boolean isValidSquare(int x, int y, int sideLength) {
        // Check if both x and y coordinates are within the absolute value of half the side length
        return Math.abs(x) <= sideLength  && Math.abs(y) <= sideLength ;
    }
