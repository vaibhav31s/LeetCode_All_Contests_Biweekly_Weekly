...
//            int[] arr = readIntArray(sc, n);
    ...
    public static int[] queryResults(int limit, int[][] queries) {
            ...
            int i = 0;
            HashMap<Integer, HashSet<Integer>> colorToIndexs = new HashMap<>();
            for (int[] x : queries) {
                int index = x[0];
                int color = x[1];
                int curColor = indexToColor.getOrDefault(index, -1);


                if (curColor == 0) {
                    indexToColor.put(index, color);
                    colorToIndexs.putIfAbsent(color, new HashSet<>());
                    colorToIndexs.get(color).add(index);
                } else {
                    colorToIndexs.get(curColor).remove(index);
                    if (colorToIndexs.get(curColor).isEmpty())  {
                        colorToIndexs.remove(curColor);
                    }
                    indexToColor.put(index, color);
                    colorToIndexs.putIfAbsent(color, new HashSet<>());
                    colorToIndexs.get(color).add(index);
                }
                answer[i++] = colorToIndexs.size();
            }
    ...
    public int[] occurrencesOfElement(int[] nums, int[] queries, int x) {
        ...
        for (int i = 0; i < queries.length; i++) {
            if (queries[i] > map.get(x).size()) {
                answer[i] = -1;
            }else {
                answer[i] = map.get(x).get(queries[i] - 1);
            }
        }
