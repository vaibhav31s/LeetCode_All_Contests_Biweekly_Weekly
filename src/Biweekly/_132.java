...
public class _132 {
    ...
    public static void main(String[] args) {
        out.println(maxTotalReward(new int[]{1,6,4,3,2}));
        out.println(maxTotalReward(new int[]{1,1,3,3}));
    }

    public static int maxTotalReward(int[] nums) {
        sort(nums);
        Integer[] memo = new Integer[60001];
        return go(nums, 0, memo);
    }
      static int go(int[] nums, int sum, Integer[] memo) {
        int ceil = ceil(nums, 0, nums.length - 1, sum + 1);
        out.print(sum + " ");

        if (memo[sum] != null) return memo[sum] ;
        int max = 0;
        for (int i = ceil; i < nums.length; i++) {
            max = Math.max(nums[i] + go(nums, sum + nums[i], memo), max);
        }
        memo[sum] = max;
        return max;
    }
    ...
    public static int ceil(int[] a, int lo, int hi, int val) {
        while (lo <= hi) {
            int mid = lo + ((hi - lo) >> 1);
            if (a[mid] < val)
                lo = mid + 1;
    ...
    public static int floor(int[] a, int lo, int hi, int val) {
        while (lo <= hi) {
            int mid = lo + ((hi - lo) >> 1);
            if (a[mid] <= val)
                lo = mid + 1;
