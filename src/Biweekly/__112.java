...
import java.io.BufferedInputStream;import java.io.File;import java.io.FileInputStream;import java.io.InputStream;import java.util.*;
...
class __112 {
    ...
    public static void main(String[] args) {
        out.println(incremovableSubarrayCount(new int[]{25,4}));
        out.println(go(2, new int[]{25,4}));
//
//        out.println(incremovableSubarrayCount(new int[]{6,5,7,8}));
//        out.println(incremovableSubarrayCount(new int[]{8,7,6,6}));

//        out.println(largestPerimeter(new int[]{1,12,1,2,5,50,3}));
    }
...
int answer =0;
        for(int i = 0; i < nums.length; i++) {
            ...
            for(int j = i ; j < nums.length; j++) {
                ...
                int[] second =Arrays.copyOfRange(nums, j +1 , nums.length);
                int[] thrid = add(first, second);
//                print(thrid);
                int last = 0;
                for(int x : thrid) {
                    if(x <= last) continue out;
                    last = x;
                }
