...
public class BB {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            int n = sc.nextInt();
            int m = sc.nextInt();
            int[][] mat = read2dIntArray(sc, n, m);
            int[][] answer = new int[n][m];

            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                    if(isGreater(mat, i, j)) {
                        answer[i][j] = getMax(mat, i, j);
                    } else answer[i][j] = mat[i][j];
                }
            }

            for (int[] x : answer) print(x);
        }
    }

    private static int getMax(int[][] mat, int i, int j) {
        PriorityQueue<Integer> pq = new PriorityQueue<>(Collections.reverseOrder());
        if (i - 1 >= 0) pq.add(mat[i -1][j]);
        if (i < mat.length - 1) pq.add(mat[i + 1][j]);
        if (j >= 1) pq.add(mat[i][j - 1]);
        if (j < mat[i].length - 1)pq.add(mat[i][ j + 1]);

        return pq.poll();
    }

    private static boolean isGreater(int[][] mat, int i, int j) {
        int curValue = mat[i][j];
        int left = i - 1 >= 0 ? mat[i - 1][j] : Integer.MIN_VALUE;
        int right = i + 1 < mat.length ? mat[i + 1][j] : Integer.MIN_VALUE;
        int up = j - 1 >= 0 ? mat[i][j - 1] : Integer.MIN_VALUE;
        int down = j + 1 < mat[i].length ? mat[i][j + 1] : Integer.MIN_VALUE;
//        if (curValue > )
        return curValue > left && curValue > right && curValue > up && curValue > down  ;
    }
