...
public class CC {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            ...
            int m = sc.nextInt();
            String s = sc.next();
            int[] arr = readIntArray(sc, m);
            String ss = sc.next();
            sort(arr);
            TreeMap<Integer, Character> map = new TreeMap<>();
            char[] sec = ss.toCharArray();

            TreeMap<Character, Integer> charss = new TreeMap<>();
            for (char ch  : sec) {
                charss.put(ch, charss.getOrDefault(ch, 0) + 1);
            }

            for (int x : arr) {
                if (map.containsKey(x)) continue ;
                map.put(x, charss.firstKey());
                charss.put(charss.firstKey(), charss.get(charss.firstKey()) - 1);
                if (charss.get(charss.firstKey()) == 0) charss.remove(charss.firstKey());
            }
            char[] ans = s.toCharArray();
//            for (int i = 0; i < n; i++)
            for (Map.Entry<Integer, Character> mm : map.entrySet()) {
                ans[mm.getKey() - 1] = mm.getValue();
            }
