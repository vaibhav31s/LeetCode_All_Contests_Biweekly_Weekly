...
//            int[] arr = readIntArray(sc, n);
    ...
    private static void go(String s, int ind, int count, long total, boolean isUsed) {

        if (!isUsed) {
            if (ind + 1 <= s.length()) {
                String ss = s.substring(ind, ind + 1);

                go(s, ind + 2, count - 1, total + Long.parseLong(ss), false);
                go(s, ind + 2, count - 1, total * Long.parseLong(ss), false);
            }
            if (ind + 2 <= s.length()) {
                String sss = s.substring(ind, ind + 2);
                go(s, ind + 3, count - 1, total + Long.parseLong(sss), true);
                go(s, ind + 3, count - 1, total * Long.parseLong(sss), true);
            }
        } else {
            if (ind + 1 <= s.length()) {
                ...
                go(s, ind + 2, count - 1, total + Long.parseLong(ss), true);
                go(s, ind + 2, count - 1, total * Long.parseLong(ss), true);
            }
        }
