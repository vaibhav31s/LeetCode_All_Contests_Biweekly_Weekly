...
//                        for(long[] xxx : dp)
//                out.println(Arrays.toString(dp[x]));
            out.println(dp[n][x] % mod);
        }
    }

    private static long go(int target, int[] arr, int index, long[][] dp) {
        if (target == 0) return 1;
        if (target < 0) return 0;
        if(dp[target][index] != 0) return dp[target][index];
        long ways = 0;
        for (int i = index; i < arr.length; i++) {
            ways += go(target - arr[i], arr, i, dp);
            ways %= mod;
        }
