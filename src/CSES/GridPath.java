...
//            }
            ...
            //right or down right = i, j+1 down = i+1, j
//            out.println(go(maze, 0, 0, n, dp));
//            out.println(Arrays.toString(dp[n]) + 1);
            if(maze[n - 1][n - 1] == '*') {
                out.println(0);
                continue outer;
            }
    ...
    private static long go(char[][] maze, int x, int y, int n, long[][] dp) {
        if (x >= n || y >= n || maze[x][y] == '*') return 0;
        if(x == (n - 1) && y == (n - 1)) return 1;
        if (dp[x][y] != -1) return dp[x][y];
        long right =   go(maze, x, y + 1, n, dp) % mod;
        long down =    go(maze, x + 1, y, n, dp) % mod;
