...
//
            ...
            for (int i = 1; i <= x; i++ ) {
                ...
                for(int xx :  arr) {
                    if(i - xx < 0) continue;
                    ways = Math.min(dp[i - xx] + 1, ways);
                }
                dp[i] = ways;
            }
            if(dp[x] == Integer.MAX_VALUE) out.println(-1);
            else out.println(dp[x]  - 1);

        }
    }

    private static long go(int target, int[] arr, long[] dp) {
        if (target == 0) return 1;
        if(target < 0) return Integer.MAX_VALUE;
        if(dp[target] != Integer.MAX_VALUE) return dp[target];
        long ways = Integer.MAX_VALUE;
        for(int x :  arr) {
            ways = Math.min(go(target - x, arr, dp) + 1, ways);
        }
