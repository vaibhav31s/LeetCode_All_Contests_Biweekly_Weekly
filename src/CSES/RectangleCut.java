...
//            int[] arr = readarr(sc, n);
            ...
            for(int h = 0; h <= hh; h++) {
                for(int w = 0; w <= ww; w++) {
                    ...
                    for(int i  = 1; i < h; i++) {
                        int first = dp[i][w];
                        int second = dp[h - i][w];
                        dp[h][w] =   Math.min(dp[h][w], 1 + first + second);
                    }
            ...
            out.println(dp[hh][ww]);

        }
    }

    private static int go(int h, int w, int[][] dp) {
        ...
        for(int i  = 1; i < h; i++) {
            int first = go(i, w, dp);
            int second = go(h - i, w, dp);
            min = Math.min(min, 1 + first +  second);
        }

        //horizontal cuts
        for(int i  = 1; i < w; i++) {

            min = Math.min(min, 1 + go(h, i, dp)  +  go(h, w - i, dp));
        }
        return   min;
    }
