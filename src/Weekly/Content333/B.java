...
//        int M = Integer.parseInt(st.nextToken());
    ...
    public int[] divisibilityArray(String word, int m) {
        ...
        for(int i = 0; i < answer.length; i++){
            answer[i]=  check(word.substring(0,i),m) ? 1 : 0;
        }
    ...
    private boolean check(String substring, int m) {
        BigInteger b = new BigInteger(substring);
        return b.mod(new BigInteger(String.valueOf(m))).equals(BigInteger.ZERO);
    }
