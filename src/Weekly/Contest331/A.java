...
public class A {
    public static void main(String[] args) {
        ...
        int a = squareFreeSubsets(new int[]{3,4,4,5});
        out.println(a);

    }
    public static int squareFreeSubsets(int[] nums) {
        int[] sqr = {1, 2, 3, 5, 6, 7, 10, 11, 13, 14, 15, 17, 19, 21, 22, 23, 26, 29, 30, 31};
        int mod = 1000000007;
    ...
    private static int go(TreeSet<Integer> set, Integer n) {
        if(set.contains(n)) return 1;
        Integer  h = set.ceiling(n);
        int a = Integer.MAX_VALUE, b=  Integer.MAX_VALUE;
        if(h != null)
             a =  go(set,  h - n );
        Integer l = set.floor(n);
        if(l  != null)
            b =  go(set, n - l );
//        if(a == Integer.MAX_VALUE && b == Integer.MAX_VALUE) return 0;
