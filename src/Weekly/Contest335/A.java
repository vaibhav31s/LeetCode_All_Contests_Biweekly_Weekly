...
public class A {
    public static void main(String[] args) {
        ...
        out.println(findValidSplit1(arr));
    }
    public static int findValidSplit(int[] nums) {
        List< BigInteger> pre = new ArrayList<>();
        BigInteger p = BigInteger.ONE;
        pre.add(p);
        int a = pre.size();
        for(int x  : nums){
            p = pre.get(pre.size()-1).multiply(new BigInteger(String.valueOf(x)));
            pre.add(p);

        }
        out.println(pre);
        List< BigInteger> suf = new ArrayList<>();
         p = BigInteger.ONE;
         suf.add(p);
        for(int i = nums.length-1; i >= 0; i--){
            int x = nums[i];
             p = suf.get(suf.size()-1).multiply(new BigInteger(String.valueOf(x)));
            suf.add(p);
        }
        for(int i = 1; i < nums.length; i++){
            out.println(pre.get(i) + " " + suf.get(nums.length-i));
            out.println(pre.get(i).gcd(suf.get(nums.length- i)));
            if(pre.get(i).gcd(suf.get(nums.length- i)).equals(BigInteger.ONE)) return i-1;
        }
    ...
    public static int findValidSplit1(int[] nums) {
        int MOD = 1000000007;
        long p = 1;
        ...
        ArrayList<Long> suf = new ArrayList<>();
        suf.add(1L);
        p=1;
        for (int i = nums.length-1; i >=0; i--) {
            p = (p * nums[i]) % MOD;
        ...
        for(int i = 1; i < nums.length; i++){
            out.println(pre.get(i) + " " + suf.get(nums.length-i));
//            out.println(pre.get(i).gcd(suf.get(nums.length- i)));
            if(gcd(pre.get(i),suf.get(nums.length-i)) == 1) return i-1;
//            if(pre.get(i).gcd(suf.get(nums.length- i)).equals(BigInteger.ONE)) return i-1;
        }
        out.println(pre);
        out.println(suf);
