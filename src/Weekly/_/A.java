...
//        out.println(maximumEnergy(new int[]{-2,-3,-1} , 2));
...
//        return maxScore1(matrix);
        return maxScore1(new int[][]{{4,3,2},{3,2,1}});
    }

    private static int maxScore1(int[][] matrix) {
        int maxScore = Integer.MIN_VALUE;

        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                if (i  + 1  < matrix.length) {
                    maxScore = Math.max(maxScore, matrix[i + 1][j] - matrix[i][j]);
                }
                if (j  + 1  < matrix[0].length) {
                    maxScore = Math.max(maxScore, matrix[i][j + 1] - matrix[i][j]);
                }
            }
        }
        return maxScore;
    ...
    private static int go(int[][] matrix, int ii, int jj, int count) {
