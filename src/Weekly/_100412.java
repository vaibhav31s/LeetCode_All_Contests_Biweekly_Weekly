...
////        print(getFinalState(nums, 2, 1000000));
//        print(getFinalState(new int[]{4,2,4}, 3, 2));
//

//[161209470]
//56851412
//39846
        print(getFinalState(new int[]{161209470}, 56851412, 39846));
    }
...
//        PriorityQueue<KeyValue<Integer, Integer>> kvs = new PriorityQueue<>((a, b)->a.value -b.value);

        TreeMap<Long, TreeSet<Integer>> tMap = new TreeMap<>();

        for (int i = 0; i < nums.length; i++) {
            long key = nums[i];
            tMap.putIfAbsent(key, new TreeSet<>());
            tMap.get(key).add(i);
        }

        while (k > 0) {
            Map.Entry<Long, TreeSet<Integer>> kv = tMap.pollFirstEntry();
            int occ = kv.getValue().size();
            long newVal = (kv.getKey() * multiplier) % mod;
            TreeSet<Integer> curValue = kv.getValue();

            if (k - occ > 0) {
                tMap.putIfAbsent(newVal, new TreeSet<>());
                tMap.get(newVal).addAll(kv.getValue());
                k -= occ;
            } else {
                TreeSet<Integer> set = new TreeSet<>();
                for (int i = 0; i < k; i++) {
                    set.add(curValue.pollFirst());
                }

                tMap.put(kv.getKey(), curValue);
                tMap.put(newVal, set);
                k = 0;
                break;
            }
        ...
        for (Map.Entry<Long, TreeSet<Integer>> kv : tMap.entrySet()) {
            int kk = Integer.parseInt(String.valueOf(kv.getKey()));
            for (int ind : kv.getValue()) {
                nums[ind] = kk;
            }
        }
    ...
    public static int countPairs(int[] nums) {
        ...
        for (int i =0; i < nums.length; i++) {
            for (int j = i + 1; j < nums.length; j++) {
                if (isGood(nums[i], String.valueOf(nums[j])) || isGood(nums[j],
    ...
    private static boolean isGood(int num, String num1) {
        char[] arr = num1.toCharArray();
        for (int i = 0; i < arr.length; i++) {
            for (int j = i + 1; j < arr.length; j++) {
                swap(arr, i, j);
                if (num == Integer.parseInt(String.valueOf(arr))) return true;
                swap(arr, i, j);
            }
        }
        if (num == Integer.parseInt(String.valueOf(arr))) return true;
