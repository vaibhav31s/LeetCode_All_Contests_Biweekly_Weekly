...
public class B {
    public static void main(String[] args) {
        ...
        int t = 1;
//        long ans  = matrixSumQueries(3, new int[][]{{0,0,1},{1,2,2},{0,2,3},{1,0,4}});
//        out.println(ans);

        out.println((Math.abs('z'-'a') % 25));
        out.println((('b'-'a') % 25));
    }
    ...
    public static long matrixSumQueries(int n, int[][] queries) {
        ...
        HashMap<Integer,Long> col = new HashMap<>();
        long sum = 0;
        for(int[] x : queries){
            if(x[0] == 1){

                sum -= col.getOrDefault(x[1],0L);
                col.put(x[1], (long) x[2]);
                colSum += x[2];
                sum-= rowSum    ;
                ...
                sum+= row.getOrDefault(x[1],0L);
                row.put(x[1], (long) x[2]);
                rowSum += x[2];
                sum-= colSum    ;
            }
            sum+= ((long) n * x[2]);
