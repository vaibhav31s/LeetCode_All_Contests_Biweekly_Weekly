...
public class B {
    public static void main(String[] args) {
        maxStrength(new int[]{3,-1,-5,2,5,-9});
    }
    public static long maxStrength(int[] nums) {
        // Arrays.sort(nums);
        HashMap<Integer, Long> map  = new HashMap<>();

        long ans =   go(nums, nums.length-1, map);
        out.println(ans);
        out.println(map);
        return ans;
    }
    static long go(int[] nums, int i, HashMap<Integer,Long> map ){
        if(i <0) return 1;
        if(map.containsKey(i)) return map.get(i);

        long gp = go(nums, i-1, map);
        if(gp == -15){
            gp = 1;
        }
        long pick = (long) nums[i] * gp;
        long notPick = gp;
//        map.put(i, Math.max(pick, notPick));
