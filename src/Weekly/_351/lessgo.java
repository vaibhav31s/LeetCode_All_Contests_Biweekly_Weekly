...
//        out.println(countBeautifulPairs(new int[]{2,5,1,4}));
        out.println(makeTheIntegerZero(3,-2));
//        out.println(power
//
//
//        (2, 60, mod) + " " + Long.MAX_VALUE);

//        for(int i = 2; i <= 2 ; i++){
//            for(int j = -2; j <= -2; j++){
//                out.print( i + " "  + j + " ");
//                out.println(makeTheIntegerZero(i, j));
//            }
//        }
//        out.println(numberOfGoodSubarraySplits(new int[]{0,1,0}));
    }
    ...
    public static int numberOfGoodSubarraySplits(int[] nums) {
        ...
        if(seg.query(0, nums.length) == 1) ans++;

        for(int i = 0; i < nums.length; i++){
            if(seg.query(0,i) == 1 && seg.query(i+1, nums.length) == 1 ){
                ans++;
                ans%= mod;
            }
        }
...
//            2i + num2
        ...
        while(!dq.isEmpty()){
            ...
            while(n-->0){
                long nums1 = dq.poll();
                if(vis.contains(nums1)) continue;
                vis.add(nums1);
//                long[] arr= new long[61];
                for(int i = 0; i <= 60; i++){
                    long ans = nums1 - operation(i, num2);
//                    arr[i] = ans;
                    if(ans == 0) return answer;
                    dq.add(ans);
                }
...
//        int answer = intCeil(num1, Math.abs(num2)+1);
    ...
    public static long operation(int i, int nums2){
        long p = power(2, i, mod );
        return p + nums2;
