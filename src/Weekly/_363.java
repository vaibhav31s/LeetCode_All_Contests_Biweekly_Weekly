...
public class _363 {
    public static void main(String[] args) {
        ...
        List<Integer> stock = Arrays.asList(1, 1);
        List<Integer> cost = Arrays.asList(5, 5);

        int maxNumberOfAlloys = maxNumberOfAlloys(n, k, budget, composition, stock, cost);

        System.out.println("The maximum number of alloys that can be produced is: " + maxNumberOfAlloys);
    }
    public static int maxNumberOfAlloys(int n, int k, int budget, List<List<Integer>> composition, List<Integer> stock, List<Integer> cost) {
        ...
        for(int i = 0; i < composition.size(); i++){
            int tempSum = 0;
            List<Integer> deepCopy = new ArrayList<>(composition.get(i));

            for (int j = 0; j < n; j++) {
                if(stock.get(j) <= 0) {
                    tempSum +=  composition.get(i).get(j) * cost.get(j);
                    continue;
                }
                int total = stock.get(j) - composition.get(i).get(j);
                tempSum += Math.abs(total) * cost.get(j);
                stock.set(j, Math.max(0, total));
            }
                max = Math.max(max,1 + maxNumberOfAlloys(n, k, (budget - tempSum), composition, stock, cost));

                max = Math.max(max,maxNumberOfAlloys(n, k, (budget - tempSum), composition, stock, cost));

//            out.println(budget);
            composition.set(i, deepCopy);
    ...
    public static int countWays(List<Integer> nums) {
        ...
        int selected = 1;
        for(int i = 0; i < n; i++) {
            if(nums.get(i) < selected && (i+1 < n && selected > nums.get(i + 1))) count++;
            selected++;
        }
