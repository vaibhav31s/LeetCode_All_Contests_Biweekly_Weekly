...
public class _374 {
    public static void main(String[] args) {
        ...
        out.println(countCompleteSubstrings("igigee",2));
        out.println(countCompleteSubstrings("aaabbbccc",3));
        out.println(countCompleteSubstrings("ba",1));//5
    ...
    public static int countCompleteSubstrings(String word, int k) {
        ...
        for (int i = 0; i < word.length(); i++) {
            ...
            int last = word.charAt(i) - 'a';

            for (int j = i; j < word.length(); j++) {
                int ind = (word.charAt(j) - 'a');
                freq[ind]++;
                if (freq[ind] > k || (Math.abs(last - (word.charAt(j) - 'a'))) > 2)
                    break;

                else if (freq[ind] == k && isGood(freq, k))
                    answer++;

                last = word.charAt(j) - 'a';
    ...
    static boolean isGood(int[] freq, int k) {
        for (int x : freq) {
