...
class _389 {
    ...
    public static void main(String[] args) {
       long answer = countSubstrings("zzz", 'z');
        out.println(minimumDeletions("ahahnhahhah" ,1));
    }
    public static long countSubstrings(String s, char c) {
        return countSubstr(s, s.length(), c, c);
    }
    static long countSubstr(String s, int n,
                           char x, char y)
    {
    ...
    public static int minimumDeletions(String word, int k) {
        ...
        int i = 0;
        for (int val :  map.values()) freq[i++] = val;
        sort(freq);
        return minDeletions(freq, k);
    }

    private static int minDeletions(int[] freq, int k) {
        int ans = 0;
        ...
        for (int i = 0; i < freq.length; i++) {
            ...
            for (int j = i + 1; j < freq.length; j++) {
                ...
                if (Math.abs(freq[j] - freq[i]) > k) {
                    int min = Integer.MAX_VALUE;
                    int jj = freq[j];

                    freq[j] =  freq[j] < k ? 0 :  freq[j] - (Math.abs(freq[j] - freq[i]) - k);
                    if (freq[j] < 0) {
                        freq[j] = jj;
                        continue;
                    }
                    min = Math.min(min, jj - freq[j]);
                    minDeletions(freq, k);
                    freq[j] = jj;

                    int ii = freq[i];
                    freq[i] = freq[i] < k ? 0 :  freq[i] - (Math.abs(freq[j] - freq[i]) - k);
                    if (freq[i] < 0) {
                        freq[i] = ii;
                        continue;
                    }
                    min = Math.min(min, ii - freq[i]);
                    minDeletions(freq, k);
                    freq[i] = ii;
                    ans += min;
                }
