...
public class A {
    ...
    public static void main(String[] args) {
       boolean[] answer =  isArraySpecial(new int[]{4,3,1,6}, new int[][]{{0,2},{2,3}});
...
//        D();
    ...
    public static boolean[] isArraySpecial(int[] nums, int[][] queries) {
        ...
        SegmentTree seg = new SegmentTree(0, pref.length);
        for (int i = 0; i < pref.length; i++) seg.update(i, pref[i]);




        for (int i = 0; i < queries.length; i++) {
            answer[i] = getAnswer(queries[i][0], queries[i][1], seg, nums);
    ...
    static  boolean  getAnswer(int left, int right, SegmentTree seg, int[] nums) {
         int sum =  (seg.query(left , right))  - quarterSquare(left + 1) ;
        out.println(sum );
        if (nums[left] == 0) {
//            out.println(quarterSquare(right - left  + 1));
          return  quarterSquare(right - left + 1) == sum;
        } else {
//            out.println(quarterSquare(right - left + 2  ));
            return  quarterSquare(right - left + 2) == sum;
        }
    }

    public static int quarterSquare(int n) {
        // Using the formula a(n) = floor(n^2 / 4)
        return (n * n) / 4;
    }
