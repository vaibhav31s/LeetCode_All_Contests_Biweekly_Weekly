...
//        out.println(clearStars("aaba*"));
    ...
    public static String clearStars(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == '*') {
                ...
                map.putIfAbsent(s.charAt(i), new Stack<>() )   ;
    ...
    private static  void remove(TreeMap<Character, Stack<Integer>> map) {
        for (char ch : map.keySet()) {
            if (!map.get(ch).isEmpty()) {
                int poped = map.get(ch).pop();
                set.add(poped);
                return;
            }
        }
