...
//            nums = [3,1,4,2,5], queries = [[2,3,4],[1,0,4]]
            out.println(countOfPeaks(new int[]{3,1,4,2,5}, new int[][]{{2,3,4}, {1,0,4} }));
            out.println(countOfPeaks(new int[]{4,1,4,2,1,5}, new int[][]{{2,2,4},{1,0,2},{1,0,4}}));

        }
    }

   static public  List<Integer> countOfPeaks(int[] nums, int[][] queries) {
...
//        if (nums[0] > nums[1]) peak[0] = 1;
//        if (nums[nums.length - 1] > nums[nums.length - 2]) peak[nums.length - 1] = 1;
        ...
        for (int[] q :  queries) {
            if (q[0] == 2) {
                ...
                if (q[1] == 0) {
                    ...
                    peak[nums.length - 1] = 0;
                } else {
                    if (nums[q[1]] > nums[q[1] - 1] && nums[q[1]] > nums[q[1] + 1]) peak[q[1]] = 1;
                    else peak[q[1]] = 0;

                    int ind = q[1];
                    if ((ind - 1) >= 1 && nums[ind - 1] > nums[ind - 2] && nums[ind - 1] > nums[ind]) {
                        peak[ind - 1] = 1;
                    } else {
                        peak[ind - 1] = 0;
                    }

                    if (ind + 1 < nums.length - 1 && nums[ind + 1] > nums[ind + 2] && nums[ind + 1] > nums[ind]) {
                        peak[ind + 1] = 1;
                    } else {
                        peak[ind + 1] = 0;
                    }
