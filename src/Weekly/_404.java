...
public class _404 {
    ...
    public static void main(String[] args) {
        out.println(maximumLength(new int[]{1,2,3,4}, 2));
        out.println(maximumLength(new int[]{1,2,3,4,5}, 2));
        out.println(maximumLength(new int[]{1,4,2,3,1,4}, 3));
    }
    ...
    public static int maximumLength(int[] nums, int k) {
        ...
        for (int i = 0; i < k; i++) maps.add(new TreeSet<>());
        for (int i = 0; i < nums.length; i++) {
            maps.get(nums[i]).add(i);
            max = Math.max(max, maps.get(nums[i]).size());
        }


        for (int i = 0; i < k; i++) {
            for (int j = i + 1; j < k ; j++) {
                max = Math.max(max, go(nums, i, j, maps));
            }
        }
//        out.println(maps);
        return max;
    ...
    static int go(int[] nums, int i, int j, List<TreeSet<Integer>> maps) {
        TreeSet<Integer> first = maps.get(i);
        TreeSet<Integer> second = maps.get(j);
        if (first.isEmpty()) return second.size();
        if (second.isEmpty()) return first.size();
        int min = first.first();
        int min1 = second.first();
        int count = 0;
        if (min < min1) {
            Integer last = min;
            boolean flag = false;
            while (last != null) {
                if (!flag) {
                    last = second.ceiling(last);
                    flag = true;
                } else {
                    last = first.ceiling(last);
                    flag = false;
                }
                count++;
            }
