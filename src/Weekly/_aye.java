...
//        }
    ...
    int go(int[] nums1, int[] nums2, int n, int i, int first, int second) {
        if (i == n - 1) {
        ...
        if (nums1[i] <= nums1[n - i] && nums2[i] <= nums2[n - i]) {
            return go(nums1, nums2, n, i + 1, first, second);
        } else if (nums1[i] <= nums2[n - i] && nums2[i] <= nums1[n - 1]) {
            if (first == Integer.MAX_VALUE) {
                return go(nums1, nums2, n, i + 1, second + 1, second);
            } else {
                return go(nums1, nums2, n, i + 1, first, second);
            }
        } else {
    ...
    public List<String> findHighAccessEmployees(List<List<String>> access_times) {
        ...
        for (List<String> str :  access_times) {
            ...
            map.get(str.get(0)).add(convertToNumber(str.get(1)));
        }
        ...
        for(String ans : map.keySet()) {
            ...
            for(int i = 0; i<  list.size(); i++) {
                ...
                for(int j = i + 1; j < list.size(); j++) {
                    if(list.get(j) >= cur && list.get(j) <= last) count++;
                }
    ...
    private static int convertToNumber(String timeString) {
